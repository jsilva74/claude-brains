...
Capture each assistant turn to a durable, on-disk spool at `Stop` time (while the host is alive) so that session memory is never lost to a teardown race. The spool is the single source of truth that `distill.sh` consumes; flush-then-dispatch on `SessionEnd`/`PreCompact` and cheap `SessionStart` orphan recovery guarantee every session is eventually condensed into the store without blocking the session.
